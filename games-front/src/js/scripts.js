...
function createCardNews(data) {
    ...
    if (data.attributes.Image.data != null) {
        const image = 'http://localhost:1337' + data.attributes.Image.data.attributes.url
        ...
        element = TitleAndTextAppend(contentElement, titleElement, textElement, data) // коректная ли функция?
...
async function loadGames(category = false) {
    ...
    if (category) { games.data = games.data.filter(data => data.attributes.game_type.data.attributes.GameTypeName == category) }
...
function contentUnification(data, titleElement, textElement) {
    ...
    textElement.innerHTML = `<div>GM:${GM.Name} | Telegram:${GM.Telegram}</div> 
                             ...
                             <div>Место проведения: ${gamePlace.Name} | Адрес: ${gamePlace.Description}</div>
...
function playerPrint(players) {
    ...
    for (var i = 0; i < players.length; i++) {
        const playerName = '<li>' + players[i].attributes.Name + '|' + players[i].attributes.Telegram + '</li>'
